...
\begin{frame}{What is Kubernetes?}
  ...
  \begin{columns}[c]
    \column{0.26\textwidth}
      \centering\includegraphics[height=2.8cm]{assets/k8s-logo.png}
    \column{0.46\textwidth}
      ...
      \end{tikzpicture}
    \column{0.26\textwidth}
      \centering\includegraphics[height=2.4cm]{assets/cncf-logo.png}
  \end{columns}
  \vfill
...
\begin{frame}{Why Kubernetes?}
  \begin{itemize}
    ...
    \item Automatic workload scheduling
    ...
    \item Self-healing
    \item Fully API driven
  \end{itemize}
\end{frame}

% --- K8s is fundamentally different! (traditional pipeline) ---
\begin{frame}{K8s is fundamentally different!}
  \begin{itemize}\item {\color{wsorange}\textbf{Traditional}} Continuous Delivery Pipeline\end{itemize}
  \vfill\centering
  \begin{tikzpicture}
    \node[single arrow, draw=none, fill=wsorange, text=white, minimum width=2.4cm, minimum height=1.1cm, align=center, font=\small\bfseries, inner sep=3pt] (a) {Stop Containers};
    \node[single arrow, draw=none, fill=wsorange, text=white, minimum width=2.4cm, minimum height=1.1cm, align=center, font=\small\bfseries, inner sep=3pt, right=4pt of a] (b) {Pull new image};
    \node[single arrow, draw=none, fill=wsorange, text=white, minimum width=2.4cm, minimum height=1.1cm, align=center, font=\small\bfseries, inner sep=3pt, right=4pt of b] (c) {Start DB};
    \node[single arrow, draw=none, fill=wsorange, text=white, minimum width=2.4cm, minimum height=1.1cm, align=center, font=\small\bfseries, inner sep=3pt, right=4pt of c] (d) {Start Application};
    \node[rectangle callout, rounded corners=8pt, fill=wsyellow, callout absolute pointer={(b.south)},
      font=\bfseries\small, align=center, text width=4.2cm, below=1.2cm of b] {You specify \textbf{HOW} to do the deployment};
  \end{tikzpicture}
  \vfill
\end{frame}

% --- K8s is fundamentally different! (declarative wishlist) ---
\begin{frame}{K8s is fundamentally different!}
  \begin{itemize}
    \item {\color{wsorange}\textbf{Declarative}} Deployment Workflow
    \item State a Wishlist
      \begin{itemize}
        \item ``I want version 1.2.3 running''
        \item ``I need the containers to restart on failure''
        \item ``Always have 3 running instances''
      \end{itemize}
    \item Kubernetes will try its best to fulfill your wishlist:
      \begin{itemize}\item {\color{wsorange}\textbf{Control Loop / Reconciliation Loop}}\end{itemize}
  \end{itemize}
  \begin{tikzpicture}[remember picture, overlay]
    \bubble[text width=4cm]{\pg{10.6cm}{-2.3cm}}{\pg{7.4cm}{-2.5cm}}{You specify \textbf{WHAT} should be there}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Reconciliation Loop}
  ...
  \begin{tikzpicture}[every node/.style={font=\bfseries}]
    \node[fill=wsorange, text=white, rounded corners=3pt, inner sep=7pt] (compare) at (0,1.8) {Compare};
    \node[fill=wsorange, text=white, rounded corners=3pt, inner sep=7pt] (update) at (1.7,-1.0) {Update};
    \node[fill=wsorange, text=white, rounded corners=3pt, inner sep=7pt] (observe) at (-1.7,-1.0) {Observe};
    \draw[-{Stealth[length=7pt]}, line width=1.6pt] (observe) to[bend left=38] (compare);
    \draw[-{Stealth[length=7pt]}, line width=1.6pt] (compare) to[bend left=38] (update);
    \draw[-{Stealth[length=7pt]}, line width=1.6pt] (update) to[bend left=38] (observe);
    \node[rectangle callout, rounded corners=7pt, fill=wsyellow, callout absolute pointer={(observe.west)},
      font=\small\bfseries, text width=2.9cm, align=center] at (-5.1,-1.0) {What is the current state};
    \node[rectangle callout, rounded corners=7pt, fill=wsyellow, callout absolute pointer={(compare.east)},
      font=\small\bfseries, text width=3.4cm, align=center] at (4.9,1.8) {Current state == desired state?};
    \node[rectangle callout, rounded corners=7pt, fill=wsyellow, callout absolute pointer={(update.east)},
      font=\small\bfseries, text width=3.4cm, align=center] at (5.2,-1.0) {Current state $\leftarrow$ Desired state};
    \node[rectangle callout, rounded corners=7pt, fill=wsyellow, callout absolute pointer={(observe.south)},
      font=\small\bfseries, text width=3.6cm, align=center] at (-2.4,-3.4) {Kubernetes has many of these loops!};
  \end{tikzpicture}
  \vfill
\end{frame}

% --- How to Kubernetes? ---
\begin{frame}[fragile]{How to Kubernetes?}
  \begin{columns}[c]
    \column{0.5\textwidth}
      \begin{itemize}
        \item Built around {\color{wsorange}\textbf{API Objects}}
        \item Everything in K8s has a {\color{wsorange}\textbf{name}}
        \item {\color{wsorange}\textbf{Declarative}} definition of state
          \begin{itemize}\item ``This is how I want it!'' $\rightarrow$ Kubernetes Manifests\end{itemize}
        \item CLI Tool: {\color{wsorange}\textbf{kubectl}}
        \item GUI Tool: {\color{wsorange}\textbf{FreeLens}}
      \end{itemize}
    \column{0.48\textwidth}
      \begin{yamlbox}
\begin{lstlisting}[style=wsyamlstyle, basicstyle=\ttfamily\small]
...
\begin{frame}{Kubernetes Management: \wsClusterPlatform}
  \begin{columns}[c]
    ...
    \column{0.34\textwidth}
      \centering\includegraphics[width=0.9\linewidth]{assets/rancher-logo.pdf}
  \end{columns}
\end{frame}
...
\begin{frame}{\exicon Exercise: Get Access To \wsClusterPlatform}
  \vspace{1cm}
  \begin{enumerate}
...
% --- Exercise: kubectl ---
\begin{frame}{\exicon Exercise: kubectl}
  \begin{enumerate}
...
% ===== Namespace =====
\objframe{0.15}{0.91}{0.41}{1.0}   % Namespace
...
% ===== Pod =====
\objframe{0.32}{0.46}{0.565}{0.56}  % Pod
...
% ===== Deployment =====
\objframe{0.32}{0.27}{0.565}{0.36}  % Deployment
...
% ===== LabelSelector =====
\objframe{0.78}{0.63}{0.98}{0.72}   % LabelSelector
...
\begin{frame}{\exicon Exercise: Deployment for the reservation system}
  \begin{enumerate}
    \item \sout{Task 1: configure the cluster to pull from a private registry} (only for a private registry)
...
% --- Task 1: Configure pull secrets (private registry, optional) ---
\begin{frame}{\exicon Task 1: Configure pull secrets}
  \begin{itemize}
    \item Create an access token to your registry with \texttt{read\_registry} scope
      \begin{itemize}
        \item e.g.\ in GitLab: Settings $>$ Repository $>$ Deploy tokens
        \item e.g.\ in GHCR: a token with \texttt{read:packages}
      \end{itemize}
  \end{itemize}
  \vfill\hfill\colorbox{wsorange}{\color{white}\bfseries\small Only needed for a private registry}
\end{frame}

\begin{frame}[fragile]{\exicon Task 1: Configure pull secrets}
  Create the pull secret in your namespace:
  \begin{termbox}
\begin{lstlisting}[style=wstermstyle, basicstyle=\ttfamily\scriptsize]
...
\end{lstlisting}
  \end{yamlbox}
  \begin{itemize}\item Kubernetes can now pull the images without further intervention\end{itemize}
  \vfill\hfill\colorbox{wsorange}{\color{white}\bfseries\small Only needed for a private registry}
\end{frame}

\begin{frame}{\exicon Task 2: Create reservation deployment}
  \centering\includegraphics[height=0.72\paperheight]{assets/dia-pedelec-ssd.pdf}
\end{frame}

\begin{frame}[fragile]{\exicon Task 2: reservation deployment manifest}
  \begin{columns}[c]
...
\objframe{0.32}{0.18}{0.565}{0.27}  % Service

% ISO/OSI with "Service lives here" bubble at Layer 4
\begin{frame}{ISO/OSI model}
  \centering\includegraphics[width=\textwidth,height=0.66\paperheight,keepaspectratio]{assets/dia-osi.pdf}
  \begin{tikzpicture}[remember picture, overlay]
...
\begin{frame}{\exicon Exercise: Service for the reservation system}
  \begin{enumerate}
...
\begin{frame}{\exicon Task 1: Create reservation service}
  \centering\includegraphics[height=0.72\paperheight]{assets/dia-pedelec-ssd.pdf}
\end{frame}

\begin{frame}[fragile]{\exicon Task 1: reservation service manifest}
  \begin{columns}[c]
    \column{0.52\textwidth}
...
% ===== Ingress =====
\objframe{0.315}{0.01}{0.565}{0.10}   % Ingress

% ISO/OSI (Ingress at Layer 7)
\begin{frame}{ISO/OSI Model}
  \centering\includegraphics[width=\textwidth,height=0.66\paperheight,keepaspectratio]{assets/dia-osi.pdf}
  \begin{tikzpicture}[remember picture, overlay]
    \bubble{\pg{10.7cm}{-2.2cm}}{\pg{8.3cm}{-2.2cm}}{The Ingress lives here (Layer 7)}
  \end{tikzpicture}
\end{frame}
...
% Ingress build-up: service exposed
\begin{frame}{Services}
  \textbf{Kubernetes Cluster}
  \vfill\centering
  \begin{tikzpicture}
    \node[tsvc] (svc) at (2.2,2.0) {\scriptsize$\ll$Service$\gg$\\Reservation};
    \foreach \x/\n in {0/1,1.5/2,3/3,4.5/4} \node[tpod] (p\n) at (\x,0) {\scriptsize$\ll$Pod$\gg$\\Reservation};
    \foreach \n in {1,2,3,4} \draw (svc) -- (p\n);
  \end{tikzpicture}\vfill
\end{frame}

% Ingress build-up: single service route
\begin{frame}{Services}
  \textbf{Kubernetes Cluster}\hfill\texttt{\wsIngressBase}
  \vfill\centering
  \begin{tikzpicture}
    \node[ting] (ing) at (2.2,3.4) {\scriptsize$\ll$Ingress$\gg$\\Pedelec};
    \node[tsvc] (svc) at (2.2,1.7) {\scriptsize$\ll$Service$\gg$\\Reservation};
    \foreach \x/\n in {0/1,1.5/2,3/3,4.5/4} \node[tpod] (p\n) at (\x,0) {\scriptsize$\ll$Pod$\gg$\\Reservation};
    \draw (ing) -- node[right, font=\tiny]{\texttt{/reservation}} (svc);
    \foreach \n in {1,2,3,4} \draw (svc) -- (p\n);
  \end{tikzpicture}\vfill
\end{frame}
...
\begin{frame}{\exicon Exercise: Ingress for the Pedelec server}
  \begin{enumerate}
